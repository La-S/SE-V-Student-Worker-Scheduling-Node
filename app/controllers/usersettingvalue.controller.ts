...
exports.create = async (req: pkg.Request, res: pkg.Response) => {
    ...
    const data: UserSettingValue = await UserSettingValue.create(req.body);
    res.send(data);
}
...
exports.update = async (req: pkg.Request, res: pkg.Response) => {
    ...
    //throws error if not found
    const userSettingValue: UserSettingValue = await getOneForId(UserSettingValue, id);
    ...
    const setting: Setting = await getOneForStringId(Setting, userSettingValue.dataValues.settingCode);
    ...
    let updatedUserSettingValue: UserSettingValue = await getOneForId(UserSettingValue, id);
...
exports.findOne = async (req: pkg.Request, res: pkg.Response) => {
    const id: number = parseInt(req.params.id, 10);
    const userSettingValue: UserSettingValue = await getOneForId(UserSettingValue, id);
    const data: UserSettingValue = await UserSettingValue.findOne({
        where: { id: id },
        include: [
            {
                model: Setting,
...
export async function getUserSettingValue(userId: number, settingCode: string): Promise<Model<any, any>> {
    const userSettingValue: UserSettingValue = await UserSettingValue.findOne({
        where: {
            userId: userId,
    ...
    const settingValue: UserSettingValue | null = await UserSettingValue.findOne({
        where: {
            userId: userId,
            settingCode: settingCode
        },
